...
def loss_original(feat_p, feat_q, x_p, x_q, sigma, sigma0, ep, lam=10 ** (-6)):
    ...
    mmd2, varEst, _ = mmd_and_var(
        feat_p, feat_q, x_p.view(m, -1), x_q.view(n, -1), sigma0, sigma, ep
    )
...
def loss_log_original(feat_p, feat_q, x_p, x_q, sigma, sigma0, ep, lam=10 ** (-6)):
    ...
    return_val = -torch.log(mmd2 + numerical_eps) + 0.5 * torch.log(varEst + lam)
    return return_val
...
def loss_mmd2(feat_p, feat_q, x_p, x_q, sigma, sigma0, ep, biased=False):
    ...
    m, n = x_p.shape[0], x_q.shape[0]
    mmd2, _, _ = mmd_and_var(
        feat_p, feat_q, x_p.view(m, -1), x_q.view(n, -1), sigma0, sigma, ep, biased=biased
    )
...
class Trainer:
    ...
    def eval_basic(self, dataloader_p, dataloader_q, num_repetitions, num_permutations=100):
    ...
    def performance_measures(self, dataloader_p, dataloader_q, num_batches, num_permutations=100):
        ...
        for idx in range(num_batches):
            ...
            m, n = x_p.shape[0], x_q.shape[0]
            mmd2, varEst, _ = mmd_and_var(
                feat_p, feat_q, x_p.view(m, -1), x_q.view(n, -1), sigma0_u, sigma, ep
            )
            mmd2_h1.append(mmd2)
            var_h1.append(varEst)

            mmd2_suth, varEst_suth = mmd_and_var(
                feat_p,
                feat_q,
                x_p.view(m, -1),
                x_q.view(n, -1),
        ...
        results = {
            ...
            "type_1_err": reject_rate_h0,
            "MMD2": mmd2_h1_sample_mean,
            "MMD2_var_calc": mmd2_h1_sample_mean_varEst,
            "MMD2_suth": mmd2_h1_sample_mean_suth,
            "MMD2_var_calc_suth": mmd2_h1_sample_mean_varEst_suth,
    ...
    def val_step(self, global_step, generate_img_grid=True):
        ...
        val_results = self.performance_measures(
            self.valloader_p, self.valloader_q, num_val_batches, num_permutations
        )
        train_results = self.performance_measures(
        ...
        if self.use_tensorboard:
            ...
            self.writer.add_scalar(
                "val_mmd2_var_suth", val_results["MMD2_var_calc_suth"], global_step
            )
            self.writer.add_scalar(
                "val_mmd2_var_batch", val_results["MMD2_var_batch"], global_step
            )
            ...
            self.writer.add_scalar(
                "train_mmd2_var_calc", train_results["MMD2_var_calc"], global_step
            )
            self.writer.add_scalar(
                "train_mmd2_var_suth", train_results["MMD2_var_calc_suth"], global_step
            )
            self.writer.add_scalar(
                "train_mmd2_var_batch", train_results["MMD2_var_batch"], global_step
            )
    ...
    def check_if_already_trained(self):
        ...
        already_trained = (
            ...
            and os.path.exists(self.trainres_path)
            and os.path.exists(self.valres_path)
        )
