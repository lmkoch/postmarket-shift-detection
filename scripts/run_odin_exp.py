    ...
    for temper in temperatures:
        for epsi in epsilons:
            ...
            for method in ["base", "odin"]:
                roc_auc, fpr95 = odin.evaluate_scores(
                    df_in[df_in["method"] == method]["score"],
...
def plot_gridsearch_results(df, temperatures, epsilons, log_dir):
    ...
    for measure in ["rocauc", "fpr95"]:
        ...
        grid_z0 = griddata(
            subset[["temperature", "epsilon"]], subset[measure], (X, Y), method="nearest"
        )
        ...
        ax = sns.heatmap(grid_z0, annot=True, linewidths=0.5, cmap=cmap, vmin=vmin, vmax=vmax)
...
def eval_best_param(dl_in, dl_out, model, temper, epsi, results_csv):
    ...
    for method in ["base", "odin"]:
        roc_auc, fpr95 = odin.evaluate_scores(
            df_in[df_in["method"] == method]["score"], df_out[df_out["method"] == method]["score"]
        )
...
def main(
    ...
    if run_gridsearch:
        ...
        run_grid_search(
            dl_in, dl_out, model, temperatures, epsilons, num_img, results_gridsearch_csv
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help="config file",
        default="./config/odin_mnist_no5_10outputs.yaml"
    ...
    parser.add_argument(
        ...
        dest="run_gridsearch",
        action="store_true",
        help="gridsearch flag",
        default=False,
    )
    parser.add_argument(
        "--run_plot", dest="run_plot", action="store_true", help="plot flag", default=False
    )
    parser.add_argument(
        "--run_eval", dest="run_eval", action="store_true", help="eval flag", default=False
    )
