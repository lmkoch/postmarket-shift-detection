...
def _mmd2_and_variance_sutherland(K_XX, K_XY, K_YY, biased=False):
    ...
    var_est = (
          2 / (m**2 * (m-1)**2) * (
              2 * torch.dot(Kt_XX_sums,Kt_XX_sums) - Kt_XX_2_sum
            + 2 * torch.dot(Kt_YY_sums,Kt_YY_sums) - Kt_YY_2_sum)
        - (4*m-6) / (m**3 * (m-1)**3) * (Kt_XX_sum**2 + Kt_YY_sum**2)
        + 4*(m-2) / (m**3 * (m-1)**2) * (
              torch.dot(K_XY_sums_1,K_XY_sums_1)
            + torch.dot(K_XY_sums_0,K_XY_sums_0))
